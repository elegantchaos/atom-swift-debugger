...
module.exports = (SwiftDebugger = {
    ...
    */
    ...
    activate(state) {
        ...
        atom.workspace.addOpener( (url) => {
            ...
            if (url == 'atom://swift-debugger/console') {
        ...
        return this.subscriptions.add(atom.commands.add('atom-workspace', {
            ...
            'swift-debugger:debug': this.doWithUpdate((view) => view.runApp()),
            'swift-debugger:stop': this.doWithUpdate((view) => this.view(true).stopApp()),
            'swift-debugger:clear': this.doWithUpdate((view) => view.clearOutput()),
            'swift-debugger:step-over': this.doWithUpdate((view) => view.stepOverBtnPressed()),
            'swift-debugger:step-out': this.doWithUpdate((view) => view.stepOverBtnPressed()),
            'swift-debugger:step-in': this.doWithUpdate((view) => view.stepOverBtnPressed()),
            'swift-debugger:resume': this.doWithUpdate((view) => view.resumeBtnPressed()),
        }));
    },
    ...
    */
    ...
    doWithUpdate(action) {
        return () => {
            if (this._view == null) {
                // if the view doesn't exist, make it first
                atom.workspace.open('atom://swift-debugger/console').then( (view) => {
                    this._view = view;
                    action(view);
                    this.updateToolBar();
                });
